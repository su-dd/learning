...
#define RED 0
    ...
    private:
...

        {
            ...
            Node() : value(0), color(RED), leftChild(nullptr), rightChild(nullptr), parent(nullptr) {}
    ...
    private:
        ...
        {
            if (d->parent == nullptr && d->leftChild == NIL && d->rightChild == NIL)
            {
        ...
        void delete_case(Node *d)
        {
            ...
            if (d->parent->color == BLACK && d->sibling()->color == BLACK && d->sibling()->leftChild->color == BLACK && d->sibling()->rightChild->color == BLACK)
            {
                d->sibling()->color = RED;
                delete_case(d->parent);
            }
            else if (d->parent->color == RED && d->sibling()->color == BLACK && d->sibling()->leftChild->color == BLACK && d->sibling()->rightChild->color == BLACK)
            {
                d->sibling()->color = RED;
            ...
            else
            {
                ...
                {
                    if (d == d->parent->leftChild && d->sibling()->leftChild->color == RED && d->sibling()->rightChild->color == BLACK)
                    {
                        d->sibling()->color = RED;
                        d->sibling()->leftChild->color = BLACK;
                        rotate_right(d->sibling()->leftChild);
                    }
                    else if (d == d->parent->rightChild && d->sibling()->leftChild->color == BLACK && d->sibling()->rightChild->color == RED)
                    {
                        d->sibling()->color = RED;
