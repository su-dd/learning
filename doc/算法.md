...
```cpp
template<typename T>
void merge_sort_recursive(std::vector<T>& array, std::vector<T>& temp, int start, int end)
{
	if (start >= end)
		return;
	...
	merge_sort_recursive(array, temp, start1, end1);
	merge_sort_recursive(array, temp, start2, end2);
...
{
	std::vector<T> temp;
	merge_sort_recursive(array, temp, 0, array.size() - 1);
}
```
