...
悲观锁，大多是需要系统底层的支持；乐观锁是一种业务实现方式，使用CAS的方式实现。

CAS操作包含三个操作数——内存位置（V）、预期原值（A）、新值(B)。

**执行CAS有两种方式：**

1. 认为位置V应该包含值A；如果包含该值，则将B放到这个位置；否则，不要更改该位置，只告诉我这个位置现在的值即可
2. 认为位置V应该包含值A；如果包含该值，则将B放到这个位置，返回true；否则，不要更改该位置，返回false
