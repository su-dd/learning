...
| 算法 | 英文 | 理解 | 评价 |
| ---- | ---- | ---- | ---- |
| 最优算法 | OPT, Optimal replacement algorithm | 在当前页面中置换最后要访问的页面 | 不可实现，但可以用作基准 |
| 最近最少算法 | LRU, Least Recently Used |  | 比较优秀，但是很难实现 |
| 最近未使用算法 | NRU, Not Recently Used |  | 和 LRU 算法很相似 |
| 最不经常使用算法 | NFU |  | 和 LRU 很类似 |
| 先进先出算法 | FIFO, First In First Out |  | 有可能会抛弃重要的页面 |
